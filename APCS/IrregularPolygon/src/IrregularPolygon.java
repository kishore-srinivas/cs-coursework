...
public class IrregularPolygon extends Shape {
	...
	public Circle calcCentroid() {
		double x, y, xSum = 0, ySum = 0;
		for (Point2D.Double p : myPolygon) {
			xSum += p.x;
			ySum += p.y;
		}
		x = xSum / myPolygon.size();
		y = ySum / myPolygon.size();
		return new Circle(x, y, 3, Color.RED, Color.RED, (float) 1.0);
	}
	...
	public void fillSection(double x, double y) {
		...
		// fill the section of the irregular polygon in which (x,y) is located
		PShape shape;
	}
}
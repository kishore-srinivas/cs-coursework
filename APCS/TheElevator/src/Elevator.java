...
{
	private static final int ACT_LAG_TIME = 750;
	...
	public void setFloor(int f) {
		...
		// Problem 4:
		doorOpen = (!floorBits[0] && buttons[1]) || (buttons[0]&&!floorBits[1]) || (floorBits[1] && floorBits[0] && buttons[2]);

		// Problem 5:
		direction[0] = !buttons[0] && !floorBits[1] && (buttons[1] || (!buttons[1] && buttons[2])) || (!buttons[1] && buttons[2] && !floorBits[0]);
		direction[1] = (buttons[0] && ((!buttons[2] && floorBits[1] && floorBits[0]) || (!buttons[1] && !floorBits[0]))) || (buttons[2] && ((!buttons[1] && !floorBits[0]) || (!buttons[0] && !floorBits[1]))) || (!buttons[0] && buttons[1] && ((!buttons[2] && floorBits[0]) || !floorBits[1]));
